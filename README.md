...
<template-id>/
  ...
  infra/                        # Kubernetes resources for this Template
    <resource files>.yaml       # applied in the order listed in template.yaml
...
- `resources`: files applied in order, relative to the Template's directory. Kubernetes resources live under the Template's `infra/` directory, so paths are written as `infra/<file>.yaml`. One of them must define the Namespace named in `catalog.json`, and it must come first — everything else in the Template lives inside it.
