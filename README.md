...
Official catalog of Templates consumed by the Sample Catalog view in [GreenCap K8s](https://github.com/greencapk8s/greencap-k8s), under Developer Experience. A Template is a complete study application (Kubernetes resources, and optionally source code to be built) that GreenCap can deploy into a Cluster in one click.
...
```
catalog.json                  # index of every Template (read by the Sample Catalog list view)
<template-id>/
...
| `title` | Card title in the Sample Catalog |
